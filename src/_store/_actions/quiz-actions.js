import {isLastSection, quizActions, selectCurrentSectionDetails} from "../_slices/quiz-slice";
...
export const getQuizzesDetails = () => {
    return async (dispatch, getState) => {
        ...
        try {
            ...
            if (quizzesList && quizzesList.length > 0 && quizzesList[0].sections.length > 0) {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            if (typeof sectionId !== 'string') {
                ...
                console.log(quizId)
                if (quizId !== -1)
                    await deleteQuizApi(quizId, token, dispatch);
            }
        } catch (e) {
            console.log(e);
        }
