...
const quizSlice = createSlice({
    ...
    reducers: {
        setCurrentQuizId(state, action) {
            const currentQuiz = state.quizzesInfos.find(quiz => quiz.id === state.currentQuizId);
            if (currentQuiz)
                currentQuiz.id = action.payload;
        ...
        setCurrentSectionInfos(state, action) {
            ...
            const sectionIndex = state.currentQuizInfos.sections.findIndex(section => section.id === state.currentSectionId);
            if (sectionIndex !== -1) {
                state.currentQuizInfos.sections[sectionIndex].id = action.payload.sectionId;
            }
        ...
        deleteQuiz(state, action) {
            state.quizzesInfos = state.quizzesInfos.filter((quiz) => quiz.id !== action.payload);
                ...
                if (quiz.id === state.currentQuizId) {
                    return {
                        ...
                        sections: quiz.sections.map((section) => {
                            return {
                                ...section,
                                dropdownOpen: section.id === action.payload ? !section.dropdownOpen : false,
                            };
                        }),
                    };
                }
...
export const selectCurrentSectionDetails = createSelector(
    ...
    (currentSectionId, currentQuiz) => currentQuiz.sections.find(section => section.id === currentSectionId)
);

export const selectCurrentSectionOrder = createSelector(
    [selectCurrentSectionId, selectCurrentQuiz],
    (currentSectionId, currentQuiz) => currentQuiz.sections.find(section => section.id === currentSectionId)?.order || 0
);
